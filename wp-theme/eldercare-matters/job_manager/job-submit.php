<?php
/**
 * Job submission form — ECM theme override.
 *
 * Preview button removed; providers save as draft for admin review.
 *
 * Overrides: wp-job-manager/templates/job-submit.php
 *
...
<form action="<?php echo esc_url( $action ); ?>" method="post" id="submit-job-form" class="job-manager-form" enctype="multipart/form-data">
	...
	if ( isset( $resume_edit ) && $resume_edit ) {
		printf(
			'<p><strong>' . esc_html__( 'You are editing an existing job. %s', 'wp-job-manager' ) . '</strong></p>',
			'<a href="?job_manager_form=submit-job&new=1&key=' . esc_attr( $resume_edit ) . '">' . esc_html__( 'Create A New Job', 'wp-job-manager' ) . '</a>'
		);
	}
	?>
